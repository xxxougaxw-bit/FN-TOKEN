...
export default function Home() {
  ...
  if (!user) return (
    <div className="min-h-screen bg-gradient-to-br from-yellow-300 via-yellow-400 to-orange-400 flex flex-col items-center justify-center p-4">
      <h1 className="text-8xl font-black italic text-black mb-8 tracking-tighter uppercase leading-none text-center">FN TOKENS</h1>
    ...
    <div className="min-h-screen bg-gradient-to-b from-yellow-300 via-yellow-400 to-yellow-500 text-black p-4 font-sans selection:bg-black/20">
      <nav className="flex justify-between items-center max-w-6xl mx-auto mb-10 border-b border-black/10 pb-8">
        ...
        <div className="flex items-center gap-4 text-xs font-black italic">
          ...
          <button onClick={() => { supabase.auth.signOut(); setUser(null); }} className="text-black/30 hover:text-black transition-colors ml-2 uppercase text-[10px]">Logout</button>
        </div>
      </nav>
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-12">
          <div className="lg:col-span-1 space-y-6 order-2 lg:order-1 text-left">
            <h2 className="text-[10px] font-black italic uppercase text-black/40 tracking-[0.4em] ml-2">Top Ranking</h2>
            ...
            {activeTab === 'lobby' ? (
                      ...
                      <div key={m.id} className={`p-10 rounded-[48px] border transition-all ${isMyMatch ? 'border-black bg-white shadow-2xl' : 'border-black/10 bg-white/40 backdrop-blur-sm'}`}>
                        <div className="flex justify-between items-center text-left">
                          <div>
                            ...
                            <p className="text-black/50 text-[10px] font-bold uppercase mt-3 tracking-widest">Host: {m.host} {m.rule && `| ${m.rule}`}</p>
                          </div>
              ...
              <div className="space-y-6">
                ...
                <div className="grid gap-8">
                  {tournaments.map((t) => (
                    <div key={t.id} className="p-12 rounded-[56px] border border-black/10 bg-white/50 backdrop-blur-md relative overflow-hidden group shadow-lg text-left">
      ...
      {isProfileModalOpen && (
        <div className="fixed inset-0 bg-yellow-400/60 backdrop-blur-2xl flex items-center justify-center p-6 z-[110]">
          <div className="bg-white border border-black/5 p-12 rounded-[60px] w-full max-w-sm shadow-[0_30px_100px_rgba(0,0,0,0.2)] text-left">
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-yellow-400/60 backdrop-blur-2xl flex items-center justify-center p-6 z-[100]">
          <div className="bg-white border border-black/5 p-12 rounded-[60px] w-full max-w-md shadow-[0_30px_100px_rgba(0,0,0,0.2)] text-left">
            <h2 className="text-5xl font-black mb-12 italic text-black uppercase tracking-tighter leading-none">New Match</h2>
            <div className="space-y-8">

...
export default function Home() {
  ...
  if (!user) return (
    <div className="min-h-screen bg-gradient-to-br from-yellow-300 via-yellow-400 to-orange-400 flex flex-col items-center justify-center p-4">
      ...
      <button onClick={() => supabase.auth.signInWithOAuth({ provider: 'google', options: { redirectTo: window.location.origin } })} className="bg-black text-white font-black px-14 py-6 rounded-2xl hover:scale-105 transition-all italic text-xl uppercase shadow-2xl">Login with Google</button>
    </div>
  );
      ...
      <nav className="flex justify-between items-center max-w-6xl mx-auto mb-10 border-b border-black/10 pb-8">
        <div className="flex flex-col">
          ...
          <button onClick={() => setIsProfileModalOpen(true)} className="flex items-center gap-2 mt-3 group">
            ...
            <span className="text-[8px] bg-black/10 px-2 py-0.5 rounded text-black/70 font-mono font-bold uppercase">ID: {epicId}</span>
          </button>
        </div>
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex gap-2 mb-12 bg-black/10 p-2 rounded-[28px] w-fit border border-black/5 mx-auto lg:mx-0">
          <button 
            ...
            className={`px-12 py-4 rounded-[22px] text-xs font-black uppercase italic transition-all ${activeTab === 'lobby' ? 'bg-black text-yellow-400 shadow-xl' : 'text-black/50 hover:text-black'}`}
          >
            Lobby
          </button>
          <button 
            onClick={() => setActiveTab('tournament')}
            className={`px-12 py-4 rounded-[22px] text-xs font-black uppercase italic transition-all ${activeTab === 'tournament' ? 'bg-black text-yellow-400 shadow-xl' : 'text-black/50 hover:text-black'}`}
          >
            Tournaments
          </button>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-12">
          ...
          <div className="lg:col-span-1 space-y-6 order-2 lg:order-1">
            <h2 className="text-[10px] font-black italic uppercase text-black/40 tracking-[0.4em] ml-2">Whale Ranking</h2>
          ...
          <div className="lg:col-span-3 pb-32 order-1 lg:order-2">
            {activeTab === 'lobby' ? (
              <div className="space-y-6">
                <h2 className="text-[10px] font-black italic uppercase text-black/60 tracking-[0.4em] ml-2 flex items-center gap-3">
                      ...
                      <div key={m.id} className={`p-10 rounded-[48px] border transition-all ${isMyMatch ? 'border-black bg-white shadow-2xl' : 'border-black/10 bg-white/40 backdrop-blur-sm'}`}>
                        ...
                        {m.status === 'closed' && isMyMatch && (
                          <div className="mt-10 flex gap-4">
                            <button disabled={!!myReport} onClick={() => reportResult(m, 'win')} className={`flex-1 py-6 rounded-3xl font-black italic text-sm uppercase transition-all ${myReport === 'win' ? 'bg-black text-yellow-400' : 'border-2 border-black text-black hover:bg-black hover:text-white'}`}>Victory</button>
                            <button disabled={!!myReport} onClick={() => reportResult(m, 'loss')} className={`flex-1 py-6 rounded-3xl font-black italic text-sm uppercase transition-all ${myReport === 'loss' ? 'bg-red-600 text-white shadow-lg' : 'border-2 border-black/10 text-black/40 hover:border-red-600 hover:text-red-600'}`}>Defeat</button>
                          </div>
                        )}
                      </div>
                    );
              ...
              <div className="space-y-6">
                <h2 className="text-[10px] font-black italic uppercase text-black/60 tracking-[0.4em] ml-2">Ongoing Tournaments</h2>
                <div className="grid gap-8">
                  {tournaments.length > 0 ? tournaments.map((t) => (
                      ...
                      <div className="flex justify-between items-center relative z-10 text-left">
                        <div>
                          ...
                          <div className="flex gap-8">
                            <span className="text-[10px] font-black text-black/40 uppercase tracking-widest font-bold">Slots: {t.participants?.length || 0} / {t.max_players}</span>
                            <span className="text-[10px] font-black text-black uppercase tracking-widest font-mono italic">Prize Pool: {t.prize_pool?.toLocaleString()} PT</span>
                          </div>
                        </div>
                    ...
                    <div className="p-32 border-2 border-dashed border-black/10 rounded-[60px] text-center text-black/20 font-black italic uppercase text-sm tracking-[0.5em]">No Events</div>
                  )}
                </div>
              </div>
            )}
      ...
      {isProfileModalOpen && (
        <div className="fixed inset-0 bg-yellow-400/60 backdrop-blur-2xl flex items-center justify-center p-6 z-[110]">
          <div className="bg-white border border-black/5 p-12 rounded-[60px] w-full max-w-sm shadow-[0_30px_100px_rgba(0,0,0,0.2)]">
            <h2 className="text-4xl font-black mb-10 italic text-black uppercase tracking-tighter">Settings</h2>
            <div className="space-y-8 text-left">
              <input type="text" value={editName} onChange={(e) => setEditName(e.target.value)} className="w-full bg-black/5 border border-black/10 rounded-3xl p-6 text-black font-bold outline-none focus:ring-4 ring-yellow-400/50" placeholder="Display Name" />
              <input type="text" value={editEpic} onChange={(e) => setEditEpic(e.target.value)} className="w-full bg-black/5 border border-black/10 rounded-3xl p-6 text-black font-bold outline-none focus:ring-4 ring-yellow-400/50" placeholder="Epic ID" />
              <div className="flex gap-6">
                <button onClick={() => setIsProfileModalOpen(false)} className="flex-1 text-black/40 font-black py-6 uppercase text-[10px] tracking-widest">Close</button>
                <button onClick={async () => {
                  await supabase.from('profiles').update({ username: editName, epic_id: editEpic || "未設定" }).eq('id', user.id);
                  setIsProfileModalOpen(false);
                  fetchInitialData(user);
                }} className="flex-1 bg-black text-yellow-400 font-black py-6 rounded-[28px] uppercase text-[10px] tracking-widest shadow-xl">Apply</button>
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="bg-white border border-black/5 p-12 rounded-[60px] w-full max-w-md shadow-[0_30px_100px_rgba(0,0,0,0.2)]">
            ...
            <div className="space-y-8 text-left">
              ...
              <div className="flex gap-6 pt-6">
                <button onClick={() => setIsModalOpen(false)} className="flex-1 text-black/40 font-black py-6 uppercase text-[10px] tracking-widest">Cancel</button>
                <button onClick={handlePost} className="flex-1 bg-black text-yellow-400 font-black py-6 rounded-[28px] uppercase text-[10px] tracking-widest shadow-2xl">Publish</button>
              </div>
            </div>
          </div>
        </div>
